...
public class SystemInitListener implements ServletContextListener {
	...
	private void systemStartup(ServletContext servletContext) {
		...
		if (forceLoad.equalsIgnoreCase(SystemConstants.FORCELOAD_Y)) {
			log.info("*******************************************************");
			log.info("中国石油四川销售成品油分公司综合办公系统开始启动...");
			log.info("*******************************************************");
		}
		try {
		...
		log.info("********************************************");
		if (success) {
			log.info("中国石油四川销售成品油分公司综合办公系统启动成功[" + G4Utils.getCurrentTime() + "]");
			log.info("启动总耗时: " + timeSec / 60 + "分 " + timeSec % 60 + "秒 ");
		} else {
			log.error("中国石油四川销售成品油分公司综合办公系统启动失败[" + G4Utils.getCurrentTime() + "]");
